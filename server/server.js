...
app.put('/api/filaments/:id', async (req, res) => {
  try {
    ...
    await pool.execute(
      ...
      [color_name, brand || 'Bambu Lab', material || 'PLA', diameter_mm || 1.75, price_per_kg, cost_per_gram, notes || null, id]
    );
  ...
  try {
    const [rows] = await pool.execute(`
      SELECT 
        ...
        SUM((COALESCE(i.material_cost, 0) + COALESCE(i.electricity_cost, 0)) * o.quantity) as total_cost, -- Material + Electricity ONLY (no labor)
